...
elem = wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, '#wrap > section > article.art1 > div > div.art1-layout > div.layout > ul > li:nth-child(8) > a')))
time.sleep(1)
elem.click() # 관광지 클릭
time.sleep(1)
...
for i in range(0, 10):
    li = wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, f'#link_{i}'))) # 첫번째 컨텐츠 요소
    time.sleep(1)
    li.click() # 첫번째 컨텐츠 클릭
    time.sleep(1)
    ...
    args = (addr1, title, firstimage, detail)
